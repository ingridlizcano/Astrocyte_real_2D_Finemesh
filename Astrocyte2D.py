...
for n in range(num_step):
    ...
    if n in saven:
        ...
        vtkfile_glc = File('Real2D/glc/glc_2D_%s.pvd' % n)
        vtkfile_atp = File('Real2D/atp/atp_2D_%s.pvd' % n)
        vtkfile_adp = File('Real2D/adp/adp_2D_%s.pvd' % n)
        vtkfile_gly = File('Real2D/gly/gly_2D_%s.pvd' % n)
        vtkfile_pyr = File('Real2D/pyr/pyr_2D_%s.pvd' % n)
        vtkfile_lac = File('Real2D/lac/lac_2D_%s.pvd' % n)
